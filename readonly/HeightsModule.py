...
def histogram(f, p):
    ...
    def f(x, μ, σ): return np.exp(-(x-μ)**2/(2*σ**2))/np.sqrt(2*np.pi)/σ

    def fμ(x): return f(x, μ, σ)
    fμxs = fμ(xs)
    ...
    im = ax.imshow(Z, vmin=0, vmax=1, extent=[
                   xmin, xmax, ymax, 0], cmap=blueMap)
...
def contour(f, p):
    ...
    im = ax.imshow(Z, vmin=0, vmax=1, extent=[
                   μmin, μmax, σmax, σmin], cmap=blueMap)
    ...
    ax.contour(np.outer(μs, np.ones_like(σs)), np.outer(
        np.ones_like(μs), σs), z, 20, colors=[green], linewidths=1.5)

    ax.plot(p.T[0], p.T[1], 'o-', linewidth=2, color=magenta)
    ax.plot([p.T[0, -1]], [p.T[1, -1]], 'o-',
            linewidth=2, color=orange, markersize=8)

    fig.show()


histogram(lambda x, mu, sig: np.exp(-(x-mu)**2/(2*sig**2)) /
          np.sqrt(2*np.pi) / sig, [160, 15])
